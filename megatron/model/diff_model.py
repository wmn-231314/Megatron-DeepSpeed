...
class TransEncoder(MegatronModule):
    """Transformer encoder.
        Adapted from Megatron-DeepSpeed's gpt_model.py, and diffmodel.py from SMDM's lit gpt.
    """
...
class TransEncoderPipe(PipelineModule, MegatronModule):
    ...
    def __init__(
        ...
        num_tokentypes=0,
        parallel_output=True,
        attn_mask_type: AttnMaskType = AttnMaskType.prefix  # Changed to prefix for diffusion
    ):
        args = get_args()
        self.parallel_output = parallel_output
        ...
        # Transformer layers with prefix attention mask
        ...
        super().__init__(layers=self.specs,
                        loss_fn=get_cross_entropy(is_prefix=True),  # Changed to True for diffusion
                        topology=topo,
