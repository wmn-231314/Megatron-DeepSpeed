...
def _vocab_size_with_padding(orig_vocab_size, tok_args):
    """Apply the requested rules to change the size of the vocabulary"""
    if tok_args.pad_vocab_size_to is not None:
        if tok_args.pad_vocab_size_to  < orig_vocab_size:
            raise ValueError(
                f"You asked to pad the vocabulary to {tok_args.pad_vocab_size_to} when the initial vocabulary size is "
                f"{orig_vocab_size}. You can only pad to a higher value."
            )

        if tok_args.make_vocab_size_divisible_by is not None and (tok_args.pad_vocab_size_to % tok_args.make_vocab_size_divisible_by) != 0:
            raise ValueError(f"{tok_args.pad_vocab_size_to} is not divisible by {tok_args.make_vocab_size_divisible_by}")

        after = tok_args.pad_vocab_size_to
    else:
        # Pad vocab size so it is divisible by model parallel size and still having GPU friendly size.
        after = orig_vocab_size
        multiple = tok_args.make_vocab_size_divisible_by * \
            tok_args.tensor_model_parallel_size
